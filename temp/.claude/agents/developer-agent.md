...
阶段一: 代码生成
  ...
  ├── 读取现有源码（确认当前状态）
  ├── 按步骤逐条 Edit/Write 代码
  └── 项目文件同步（{{BUILD_CONFIG}} / {{PROJECT_CONFIG}} 如需）
...
阶段三: 构建验证（强制）
  ├── 检测编辑器状态 → {{HOT_RELOAD}} 或 {{FULL_BUILD}}
  └── 分析编译结果 → 通过/警告/失败
...
- 新文件 → `Write` 完整创建
- {{BUILD_CONFIG}} 依赖变更 → 只添加，不删除现有声明

### 1.4 项目文件同步

| 变更类型 | 同步操作 |
|---------|---------|
| 新建 .h/.cpp | 确认路径在 `{{SOURCE_DIR}}/<Module>/Public/` 或 `Private/` 下 |
| 新增模块依赖 | 更新 `<Module>{{BUILD_CONFIG}}` 的 `{{BUILD_DEPENDENCY_SECTION}}` |
| 新建模块 | 更新 `*{{PROJECT_CONFIG}}` 的 `Modules` 数组 + 重新生成 `.sln` |
| 纯方法实现 | 无需同步 |
...
**A. 规范符合度**：
- [ ] 类命名：{{NAMING_CONVENTION_PREFIXES}} 前缀正确
- [ ] 方法命名：PascalCase，动词开头
- [ ] bool 变量：b 前缀
- [ ] 代码注解宏完整（样板宏存在？框架生成头文件最后 include？）
...
- [ ] 资源分配后释放路径完整（FArchive* delete、文件句柄关闭）

**C. 框架运行时安全**（每个必须明确检查）：
- [ ] 资源对象指针 → 内存管理注解保护
- [ ] Tick 中代码 → 无 LoadObject/SpawnActor/大量分配
- [ ] 构造函数中 → 无虚函数调用
- [ ] 跨线程访问 → 有 {{THREAD_SAFETY_MECHANISM}} 保护
...
若 `get_status` 本身失败（MCP 桥未运行）→ 直接走 {{FULL_BUILD}} 路径（编辑器不可能在运行）。

### 步骤 2：选择编译路径

| 条件 | 路径 | 方式 |
|------|------|------|
| editorConnected: true | {{HOT_RELOAD}} | `{{HOT_RELOAD_CMD}}(wait=false)` + 轮询 `{{HOT_RELOAD_STATUS}}` (≤20×3s) |
| editorConnected: false 或 get_status 失败 | {{FULL_BUILD}} | `{{FULL_BUILD_CMD}}` |

**关键规则**: 编辑器运行时**绝不**调用 `build` action。

### 步骤 3：执行编译

**{{HOT_RELOAD}} 路径**：
```
1. {{HOT_RELOAD_CMD}}(wait=false)
2. 轮询 {{HOT_RELOAD_STATUS}} → 检查 compiling 字段
3. compiling: false → get_log(filter="{{HOT_RELOAD}}", maxLines=10)
4. 日志含 "succeeded" → PASSED / "failed"/"error" → FAILED
```

**{{FULL_BUILD}} 路径**（编辑器未运行或 MCP 桥未连接）：
```
1. {{FULL_BUILD_CMD}}
2. 阻塞等待 build 完成（{{FULL_BUILD}} 为同步 action，返回即完成）
3. 检查 build 返回值：成功 → get_log(filter="{{FULL_BUILD}}", maxLines=20) 确认无错误
4. 日志含 "error" / "failed" → 提取错误(路径+行号+错误码) → FAILED
5. 日志无错误标记 → PASSED
```
...
| 结果 | 行为 |
|------|------|
| 编译通过 + 无警告 | → PASSED，进入阶段四 |
| 编译通过 + 警告 | → MINOR issue，进入阶段四 |
| 编译失败 | → 提取错误(路径+行号+错误码) → 进入阶段四修复 |
