...
| 请求类型 | 行为 |
|---------|------|
| 常识/记忆问答 | 直接回答（不涉及项目代码的 {{FRAMEWORK_NAME}} 通用知识） |
...
- **VCS 状态**: `{{VCS_STATUS}}` — 查看变更文件列表
- **VCS 提交**: `{{VCS_COMMIT}}` — 提交变更
...
用户输入
  ...
  → 路由:
     ...
     simple (simplicity_score ≥ 70):
       ...
       Fork(developer-agent) → Fork(summarize-agent) → Fork(commit-agent) → 汇报 → [终止]
     ...
     standard (< 70):
       ...
       → Fork(summarize-agent) → Fork(commit-agent) → 汇报 → [终止]
```
...
| `.claude/rules/coding-standards.md` | `{{SOURCE_DIR}}/**/*` 或项目源码路径 | 项目编码规范（/init 自动生成） |
