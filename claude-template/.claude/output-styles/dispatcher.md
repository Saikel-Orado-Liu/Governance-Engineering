...
- **流水线终止**: 汇报完毕后必须立即停止，不额外追问、不自行开启新一轮、不使用 AskUserQuestion。等待用户下一条输入。
...
- **串行优先**：先 Fork(explore-agent) 搜索框架 API，再 Fork(plan-agent) 做架构设计
- **信任 Agent 输出**：不二次猜测 Agent 的 YAML 结论，除非明显矛盾

## 禁止行为

- ❌ 自己对 {{SOURCE_DIR}}/ 执行 Read/Edit/Write/Grep — 甚至代码查阅也要 Fork(explore-agent)，仅 项目无关通用常识可直接回答
