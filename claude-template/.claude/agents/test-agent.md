...
name: test-agent
description: >
  自动化测试生成与执行——为 developer-agent 生成的代码创建 项目框架 {{TEST_FRAMEWORK}} 测试，
...
阶段二：生成测试代码
  ├── 为每个新增/修改的公开方法生成 {{TEST_FRAMEWORK}} 测试
  ├── 测试文件命名：<ClassName>Test.cpp，放在 {{SOURCE_DIR}}/Test/Private/Tests/
  └── 更新 Test{{BUILD_CONFIG}} 添加 {{TEST_FRAMEWORK}} 模块依赖（如需）
...
2. 过滤：仅处理 .h 文件中包含 {{EXPOSE_ANNOTATION}}() 或 public 方法声明的变更
...
### 2.1 项目框架 {{TEST_FRAMEWORK}} 模板

```cpp
#include "Misc/{{TEST_FRAMEWORK}}.h"
#include "<TargetHeader.h>"

{{TEST_GUARD_START}}

{{TEST_IMPLEMENT_MACRO}}(F<ClassName>_<MethodName>_<Scenario>,
    "Test.<ClassName>.<MethodName>.<Scenario>",
    E{{TEST_FRAMEWORK}}Flags::EditorContext | E{{TEST_FRAMEWORK}}Flags::EngineFilter)

bool F<ClassName>_<MethodName>_<Scenario>::RunTest(const {{STRING_TYPE}}& Parameters)
{
    // Arrange - 准备测试数据
...
```
{{SOURCE_DIR}}/Test/Private/Tests/
├── <ClassName>Test.cpp    # 每个被测类一个文件
```

### 2.3 {{BUILD_CONFIG}} 依赖（如需）

检查 `Test{{BUILD_CONFIG}}` 是否包含 `{{TEST_FRAMEWORK}}` 模块：
- 如未包含 → 添加 `"{{TEST_FRAMEWORK}}"` 到 `{{PRIVATE_DEPS}}` 数组
- 仅添加，不删除现有声明
...
- 不测试私有实现细节
- 测试文件仅放在 {{SOURCE_DIR}}/Test/Private/Tests/ 下
- 编译失败修复 ≤2 次
