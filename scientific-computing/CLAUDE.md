...
Python 3.11+ 科学计算库 项目。多模块 "linalg/stats/interp/integrate"（src/scicomp/）依赖 numpy/scipy
...
| 请求类型 | 行为 |
|---------|------|
| 常识/记忆问答 | 直接回答（不涉及项目代码的 Python 通用知识） |
...
- **构建**: `pip install -e ".[dev]"` — editable 模式安装 + dev 依赖
- **测试**: `pytest` — pytest 运行全部测试
- **VCS 状态**: # 无版本控制
- **VCS 提交**: # 无版本控制
...
- PEP 8 + ruff（line-length=100），4 空格缩进
...
| `.claude/rules/coding-standards.md` | `src/**/*.py` 或项目源码路径 | 项目编码规范（/init 自动生成） |
