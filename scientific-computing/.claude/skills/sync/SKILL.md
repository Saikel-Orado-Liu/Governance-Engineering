...
```
dv status 2>&1
  → 非 workspace → 输出 "不在 Diversion workspace 中，同步跳过。"
  → 正常 → 继续
```
...
```bash
dv log --oneline -10 2>&1
dv diff --name-status <last_sync_commit>..HEAD 2>&1
```
