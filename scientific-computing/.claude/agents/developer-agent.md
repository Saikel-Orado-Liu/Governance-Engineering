...
阶段一: 代码生成
  ...
  ├── 读取现有源码（确认当前状态）
  ├── 按步骤逐条 Edit/Write 代码
  └── 项目文件同步（.Build.cs / .uproject 如需）

阶段二: 自审查 — 静态分析（强制）
  ├── 规范检查：命名/UPROPERTY/UE_LOG
  ├── 逻辑检查：空指针/边界/类型截断/资源泄漏
  ├── UE 安全：GC 保护/Tick 陷阱/构造函数虚函数
  └── 范围检查：实际变更符合计划
...
| 变更类型 | 同步操作 |
|---------|---------|
| 新建 .h/.cpp | 确认路径在 `Source/<Module>/Public/` 或 `Private/` 下 |
| 新增模块依赖 | 更新 `<Module>.Build.cs` 的 `Public/PrivateDependencyModuleNames` |
| 新建模块 | 更新 `*.uproject` 的 `Modules` 数组 + 重新生成 `.sln` |
| 纯方法实现 | 无需同步 |
...
**A. 规范符合度**：
- [ ] 类命名：U/A/F/I/E/T 前缀正确
- [ ] 方法命名：PascalCase，动词开头
- [ ] bool 变量：b 前缀
- [ ] UPROPERTY/UFUNCTION 宏完整（GENERATED_BODY 存在？generated.h 最后 include？）
...
- [ ] 构造函数中 → 无虚函数调用
- [ ] 跨线程访问 → 有 FCriticalSection 或 AsyncTask 保护
...
| 条件 | 路径 | 方式 |
|------|------|------|
| 安装成功 | ruff 检查 | `ruff check src/` |
| 安装成功 | mypy 检查 | `mypy src/` |
| 安装成功 | pytest 运行 | `pytest` |

### 步骤 3：修复验证失败

```
修正 ruff/mypy/pytest 报错直到全部通过（≤3 次修复循环）
```
...
**Live Coding 路径**：
```
1. live_coding_compile(wait=false)
2. 轮询 live_coding_status → 检查 compiling 字段
3. compiling: false → get_log(filter="Live Coding", maxLines=10)
4. 日志含 "succeeded" → PASSED / "failed"/"error" → FAILED
```

**UBT 路径**（编辑器未运行或 MCP 桥未连接）：
```
1. build(configuration="Development", platform="Win64")
2. 阻塞等待 build 完成（UBT 为同步 action，返回即完成）
3. 检查 build 返回值：成功 → get_log(filter="UBT", maxLines=20) 确认无错误
...
5. 缺少关键信息（如引擎 API 签名不确定且 ue-mcp 不可用）
