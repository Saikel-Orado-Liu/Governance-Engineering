...
2. Glob 发现可能遗漏的相关文件（*同名字不同路径？* *package.json/pyproject.toml？* *package.json/pyproject.toml？*）
3. Read 关键文件的完整声明（.h 公开接口、package.json/pyproject.toml 依赖列表）
...
- 每种方案的 trade-off 是什么？（性能/复杂度/可维护性/扩展性/框架惯例符合度）
- 所选方案是否与现有模块架构一致？
- 是否存在更简单但被忽略的方案？
...
- 特别关注：编译兼容性、内存管理安全、模块依赖正确性、跨线程安全
...
4. 我的风险识别是否覆盖了**编译、GC、线程安全、模块依赖**这四个 框架重点领域？□
