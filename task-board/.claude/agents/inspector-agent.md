...
- 循环边界正确（< vs <=）？
- 无隐式类型截断（int64→int32）？
- console.log 使用正确类别（非 LogTemp）？
- 资源分配后释放路径完整（无泄漏）？

**C. 架构一致性**：
- 新建文件路径符合项目模块布局？
- package.json/pyproject.toml 依赖声明与实际 import 匹配？
- 没引入计划外的跨模块依赖？

**D. Web 运行时安全**（每个必须明确检查）：
- 异步操作 → 有 Promise/async-await 正确处理
- useEffect 中 → 无无限循环风险
- API 请求 → 异常处理和 loading 状态
- 状态更新 → 不可变状态模式

### 1.2 构建验证

```
步骤 1: 前端构建 → cd frontend && npm run build
步骤 2: 后端安装 → cd backend && pip install -e .
步骤 3: 分析结果
  构建通过 + 无警告 → passed
  构建通过 + 警告   → MAJOR issue (评估是否需要修复)
  构建失败          → CRITICAL issue (提取错误位置+错误码+修复建议)
```
