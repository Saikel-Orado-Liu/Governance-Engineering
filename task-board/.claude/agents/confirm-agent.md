...
**思考语言：全部思考过程（推理、分析、决策权衡）必须使用中文。仅代码、YAML 键名、框架类名等技术标识符除外。**
...
| 2 | header_changes | 20 | 无.h变更 | 追加声明到已有.h 或 仅注释 | 新建.h 或 修改/删除已有声明 |
...
| 6 | new_module_deps | 5 | 无新依赖 | 已声明模块导入 | 需改 package.json/pyproject.toml |
| 7 | files_created | 3 | 0新建 | 1新建≤50LOC | ≥2新建或>50LOC |
| 8 | ambiguity_level | 2 | NONE | LOW | MEDIUM/HIGH |

**header_changes 判定详解**:
- **100分**: 没有任何 .h 文件变更 — 不新建、不修改
- **50分**: 仅向已有 .h 追加新声明（方法/变量/friend，不修改已有签名）或仅注释/格式化变更
- **0分**: 新建 .h 文件 或 修改/删除已有声明/签名
