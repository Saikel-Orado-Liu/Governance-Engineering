...
逐文件扫描 backend/src/ + frontend/src// 目录：

| 维度 | 检测内容 | 阈值 |
|------|---------|------|
| 未使用代码 | 未引用的类/函数/变量/#include | 0 引用 |
| 命名规范 | 无强制前缀 前缀、b 前缀、PascalCase | 违反即标记 |
...
- 搜索锁竞争模式（Mutex::Lock 在循环内）
- 搜索大量内存分配（Array::Add 无 Reserve、string 拼接循环）
...
| string 拼接在循环中且 >10 次迭代 | **标记** — 用 string::Join 或 模板字符串 |
| Array::Add 在循环中无 Reserve | **标记** — 预分配 |
| Mutex::Lock 包裹 >20 行代码 | **标记** — 缩小临界区 |
...
| Map::Find 结果在 3 行内再次 Find 同 key | **标记** — 缓存迭代器 |
...
- 搜索 Array<float>/Array<{x,y}> 逐元素运算
- 搜索可并行化的独立循环迭代
- 搜索 memcpy/memset 模式
