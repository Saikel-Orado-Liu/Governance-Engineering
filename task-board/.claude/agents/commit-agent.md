...
| 决策 | 匹配规则 |
|------|---------|
| ✅ 提交 | `backend/src + frontend/src/**` |
| ✅ 提交 | `docs/**` |
| ✅ 提交 | `.claude/**` |
| ✅ 提交 | `Config/**` |
| ✅ 提交 | `Content/**` |
| ✅ 提交 | `*package.json`, `*无（非插件项目）` |
| ✅ 提交 | `*package.json`, `无（非编译目标项目）` |
| ❌ 跳过 | `Binaries/**`, `Intermediate/**`, `Saved/**`, `DerivedDataCache/**` |
| ❌ 跳过 | `.idea/**`, `.vs/**`, `.vscode/**` |
| ❌ 跳过 | `*.sln`, `*.user`, `*.DotSettings*` |
| ❌ 跳过 | `*.dll`, `*.exp`, `*.pdb`, `*.modules` |

**防漏**: 白名单筛选后无匹配但 `git status` 有变更 → 检查遗漏。`.h`/`.cpp` 不在 `backend/src + frontend/src/` 下 → 加入列表标注 `[UNEXPECTED-LOCATION]`。
