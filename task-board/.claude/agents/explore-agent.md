...
```
Layer 0: 项目源码 (backend/src/ + frontend/src//) — Grep/Glob/Read，验证关键文件存在性和引用关系
Layer 1: 项目知识库 (docs/ai/) — 模块卡片中的已有记录
Layer 2: 框架头文件 — Grep node_modules 或 npm docs → Read node_modules（主要搜索层）
Layer 3: 框架源码 — Grep node_modules（仅前两层无法确认时）
```
...
- 依赖的模块已在 package.json 中声明，且接口已通过模块卡片确认

**搜索优先级**: `Grep`（精准符号定位）→ `Read`（完整类结构）→ `Grep`（全文搜索，Token 消耗大，最后手段）
...
explore_report:
  ...
  engine_apis:
    - {class: "<框架类名>", header: "<框架路径>", key_methods: ["<完整签名>"], source: "Grep|Read"}
...
- `engine_apis[].source`: 标注数据来源（Grep / Read / reflect_class），让下游可追溯
