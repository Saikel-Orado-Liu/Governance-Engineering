...
alwaysApply: true
paths:
  - "**/*.{h,cpp,Build.cs,cs,uproject,yaml,md,json}"
---
...
- explore-agent 和 plan-agent 串行执行：explore 先搜索引擎 API → plan 接收 explore_report 后做架构设计
...
| explore-agent 失败 | 跳过引擎搜索，plan-agent 基于训练知识继续（标注 `search_phase: independent`）；引擎 API 不可用不应阻塞架构设计 |
...
| developer-agent **BLOCKED** | 无法执行（缺少源文件/ue-mcp 不可用/需求不明确）。保存 plan_result + 已注入数据 → AskUserQuestion → 用户选择：①补充信息后重试（重新 Fork(developer-agent) 注入 plan_result + 补充信息） ②简化需求 ③放弃 |
