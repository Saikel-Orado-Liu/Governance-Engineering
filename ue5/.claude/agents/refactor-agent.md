...
- 引入新第三方依赖
- 在重构中夹带功能变更
- mcp__ue-mcp__editor 不可用时不调用 — 不阻塞
...
| 命名规范 | U/A/F/I/E/T 前缀、b 前缀、PascalCase | 违反即标记 |
...
- 搜索锁竞争模式（FCriticalSection::Lock 在循环内）
- 搜索大量内存分配（TArray::Add 无 Reserve、FString 拼接循环）
...
| FString 拼接在循环中且 >10 次迭代 | **标记** — 用 FString::Join 或 TStringBuilder |
| TArray::Add 在循环中无 Reserve | **标记** — 预分配 |
| FCriticalSection::Lock 包裹 >20 行代码 | **标记** — 缩小临界区 |
...
| TMap::Find 结果在 3 行内再次 Find 同 key | **标记** — 缓存迭代器 |
...
- 搜索 TArray<float>/TArray<FVector> 逐元素运算
- 搜索可并行化的独立循环迭代
- 搜索 memcpy/memset 模式
...
| FVector/FVector4 批量变换 | **标记** — SIMD 加速 |
