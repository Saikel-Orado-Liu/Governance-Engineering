...
```
步骤 1: mcp__ue-mcp__project(action="get_status") → 检测 editorConnected
  若 get_status 本身失败（MCP 桥未运行）→ 直接走 UBT 路径

步骤 2: 选择编译路径
  editorConnected: true  → Live Coding
    live_coding_compile(wait=false) → 轮询 live_coding_status (≤20次×3s)
    → get_log(filter="Live Coding", maxLines=10) → 搜索 succeeded/failed/error
  editorConnected: false 或 get_status 失败 → UBT
    build(configuration="Development", platform="Win64")
    → 阻塞等待完成 → get_log(filter="UBT", maxLines=20) → 搜索 error/failed
    → 提取错误(路径+行号+错误码)
