...
```
1. Grep 搜索关键符号（类名、方法名、导入模块）确认存在性和引用关系
2. Glob 发现可能遗漏的相关文件（*同名字不同路径？* *package.json？* *tsconfig.json？*）
3. Read 关键文件的完整声明（.ts 公开接口、package.json 依赖列表）
4. 涉及第三方库 API 时 → 搜索类型定义确认签名
...
- 特别关注：编译兼容性、状态管理正确性、模块依赖正确性、Canvas 渲染性能
...
- `steps[].acceptance`: 每条必须是可客观检查的条件（"构建通过"、"XX 测试用例通过"、"YY 方法返回指定类型"）
...
4. 我的风险识别是否覆盖了**构建、状态管理、模块依赖、渲染性能**这四个重点领域？□
