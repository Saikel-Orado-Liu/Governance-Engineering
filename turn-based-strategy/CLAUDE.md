<!--
  ...
  Project: turn-based-strategy
  Architecture: 纯调度者模式 — 主对话永不执行任务，100% Fork
  Updated: 2026-05-11
  Length: 目标 ≤ 180 行
-->

# turn-based-strategy — AI 调度者

## 项目概述

TypeScript 5.6 + Vite 6 + Canvas 2D 回合制策略游戏项目。单模块 src/，无外部运行时依赖。
...
| 请求类型 | 行为 |
|---------|------|
| 常识/记忆问答 | 直接回答（不涉及项目代码的 Vite 6 + Canvas 2D 通用知识） |
...
- **测试**: `npm test` — vitest run 运行全部测试
- **VCS 状态**: `git status` — 查看变更文件列表
- **VCS 提交**: `git commit` — 提交变更
...
- TypeScript strict 模式，ES2020 target，2空格缩进
...
| `.claude/rules/coding-standards.md` | `src/**/*.ts` | 项目编码规范（/init 自动生成） |
