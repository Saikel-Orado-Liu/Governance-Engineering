...
TypeScript 5.6 CLI 工具 / 静态站点生成器 项目。单模块 "src"（src/），无外部运行时依赖
...
| 请求类型 | 行为 |
|---------|------|
| 常识/记忆问答 | 直接回答（不涉及项目代码的 Node.js 通用知识） |
...
- **构建**: `npm run build` — TypeScript 编译 (tsc)
- **测试**: `npm run test` — vitest 运行全部测试
- **VCS 状态**: `git status` — 查看变更文件列表
- **VCS 提交**: `git commit <files> -m '<message>'` — 提交变更
...
用户输入
  ...
  → 路由:
     ...
     standard (< 70):
       ...
       → 触发审查? (≥3文件|.ts变更|LOC>50|新算法) → Fork(inspector-agent)
...
| inspector-agent | sonnet | 独立审查（≥3文件 或 .ts变更 或 LOC>50 或 新算法） |
...
| `.claude/rules/coding-standards.md` | `src/**/*.ts` 或项目源码路径 | 项目编码规范（/init 自动生成） |
...
**思考语言**: 全部思考过程（推理、分析、内部独白、thinking）使用中文。代码、YAML 键名、技术标识符除外。
