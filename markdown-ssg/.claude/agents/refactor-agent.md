...
3. `docs/ai/standards/ts-checklist.yaml` — 分层规范索引
...
| 维度 | 检测内容 | 阈值 |
|------|---------|------|
| 未使用代码 | 未引用的导出/函数/变量/导入 | 0 引用 |
| 命名规范 | PascalCase（类）、camelCase（函数）、UPPER_SNAKE_CASE（常量） | 违反即标记 |
| 循环依赖 | A→B→A 的模块导入链路 | 任意深度 |
...
| 条件 | 判定 |
|------|------|
| 函数在循环/定时器中被高频调用 | **自动标记** — 高频执行 |
| 函数在循环嵌套 ≥2 层中被调用 | **标记** — 潜在 N² 复杂度 |
| 函数被 ≥10 个调用点引用 | **标记** — 热点函数 |
| 模块初始化/配置加载中被调用 ≥5 次 | **标记** — 初始化瓶颈 |
...
**检测方法**：
- 搜索同步文件 I/O 调用（readFileSync, writeFileSync 等）
- 搜索锁/互斥模式（Mutex, Lock 在循环内）
- 搜索大量内存分配（Array push 无预分配、字符串拼接循环）
- 搜索热循环中未缓存的重复计算

**标记条件**：
| 条件 | 判定 |
|------|------|
| 同步文件 I/O 在循环中 | **CRITICAL** |
| 字符串拼接在循环中且 >10 次迭代 | **标记** — 用 Array.join 或模板字符串 |
| Array.push 在循环中且已知长度 | **标记** — 预分配或使用 TypedArray |
| 锁/互斥包裹 >20 行代码 | **标记** — 缩小临界区 |
...
**检测方法**：
- 搜索重复调用相同参数的纯函数
- 搜索 Map/Set 查找后未缓存结果的多层查找
- 搜索在循环中计算的静态数据（不变值在循环中重复计算）
- 搜索未使用 const 的常量返回函数
...
| Map.get 结果在 3 行内再次 get 同 key | **标记** — 缓存变量 |
| 循环中计算固定值（如字符串格式化常量） | **标记** — 提到循环外 |
| 循环内重复计算数组长度/属性访问 | **标记** — 提到循环外 |
...
| 条件 | 判定 |
|------|------|
| 数组逐元素运算（>1000 次迭代） | **标记** — 考虑 worker_threads 或流式处理 |
| 简单独立循环体无数据依赖 | **标记** — 并行化 |
| 大块 Buffer/文件操作 | **标记** — 检查是否可用流式处理 |
...
| 级别 | 标准 | 处理方式 |
|------|------|---------|
| **low** | 命名修正、未使用代码删除、预分配、常量提取 | 自动执行 |
| **medium** | 函数拆分(>40行)、降低圈复杂度(>15)、缓存结果、内联优化 | 批量提案，可一次性审批 |
| **high** | 修改公开API、模块拆分、线程模型变更、>5文件 | 逐项提案，必须人工审批 |
| **performance** | 高频循环中同步 I/O、循环中字符串拼接、缺少预分配的 N² 循环 | 标记为 performance 类型，优先级高于同级别其他 issue |
...
proposals:
  - id: RF-<N>
    ...
    proposed_change:
      ...
      files_affected: ["<路径>"]
      estimated_loc: <N>
      optimization: "<算法优化|缓存优化|并行化|内联展开|结构重组>"
