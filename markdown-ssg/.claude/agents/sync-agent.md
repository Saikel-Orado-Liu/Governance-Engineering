...
| 类别 | 匹配 | 处理 |
|------|------|------|
| 公开接口变更 | `.ts` 文件导出签名修改（函数/类新增/修改/删除） | 更新模块卡片 public_interface |
| 模块依赖变更 | `package.json` dependencies 修改 | 更新模块卡片 dependencies |
| 模块增删 | `src/` 目录新增/删除 | 更新 MODULE_INDEX.yaml |
| 纯实现变更 | `.ts` 内部实现修改 | 跳过（不影响知识库） |
